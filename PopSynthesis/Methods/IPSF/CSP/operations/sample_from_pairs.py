...
def update_by_rm_for_all_pools(rm_df: pd.DataFrame, pools: Dict[str, pd.DataFrame], check_cols:List[str]) -> Dict[str, pd.DataFrame]:
    results = {}
...
def update_by_rm_for_pool(rm_df: pd.DataFrame, target_pool: pd.DataFrame, check_cols:List[str]) -> pd.DataFrame:
    ...
    filered_pool = converted_target_pool.loc[converted_target_pool.index.isin(updated_target_comb)]
    return filered_pool.reset_index()
    

def convert_condensed_syn_back(condensed_syn: pd.DataFrame, syn_id: str) -> pd.DataFrame:
...
def condense_evidence_syn(given_syn: pd.DataFrame, syn_id: str, evidence_cols: List[str]) -> pd.DataFrame:
    ...
    syn_gb_ids = check_syn.groupby(evidence_cols)[[syn_id, COUNT_COL]].apply(lambda x: list(x.to_numpy())).rename(ID_COUNT_COL)
...
def sample_matching_from_pairs(given_syn: pd.DataFrame, syn_id: str, paired_pool: pd.DataFrame, evidence_cols: List[str], sample_cols: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    combined_condense = pd.merge(remained_condensed_syn, filtered_condensed_pool, left_index=True, right_index=True)
