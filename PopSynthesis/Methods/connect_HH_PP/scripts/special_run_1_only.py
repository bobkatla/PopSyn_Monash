...
def main():
    ...
    pool = pd.read_csv(
        os.path.join(processed_data, "keep_check", f"updated_pool_{i-1}.csv")
    )
    marg_hh = pd.read_csv(
        os.path.join(processed_data, "keep_check", f"updated_marg_{i-1}.csv"),
        header=[0, 1],
    )
    ...
    combine_df_hh_main, del_hh = get_combine_df(
        hh_df, dict_pool_sample["Main"].copy().value_counts().reset_index()
    )
    ...
    for rela in all_rela_exist:
        ...
        if pop_rela is not None:
            ...
            pop_rela[geo_lev] = pop_rela.apply(
                lambda r: dict_hhid[int(r["hhid"])], axis=1
            )
            cols_main = [
                f"{x}_main"
                for x in PP_ATTS
    ...
    if len(del_df) > 0:
        ...
        ls_del_id = list(del_df_final["hhid"].astype(str)) + list(
            del_hh["hhid"].astype(str)
        )
    else:
    ...
    if check == 0:
        ...
        for adjust_att in cols_pool:
            temp_hold = (
                hh_df_keep.groupby(geo_lev)[adjust_att]
                .value_counts()
                .unstack()
                .fillna(0)
            )
        ...
        marg_hh = marg_hh.set_index(
            marg_hh[marg_hh.columns[marg_hh.columns.get_level_values(0) == "zone_id"]]
        )
        marg_got_kept = marg_got_kept.set_index(
            marg_got_kept[
                marg_hh.columns[marg_hh.columns.get_level_values(0) == "zone_id"]
            ]
        )
        print(marg_hh)
        print(marg_got_kept)
