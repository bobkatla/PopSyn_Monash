...
def get_pool(df_seed, state_names, pool_sz, special=True):
    print("Learn BN")
...
def main():
    ...
    # Outputing
    final_pp.to_csv(
        os.path.join(output_dir, f"syn_pp_bn_adjust1_{adjust_att}_{geo_lev}.csv"),
        index=False,
    )
    final_hh.to_csv(
        os.path.join(output_dir, f"syn_hh_bn_adjust1_{adjust_att}_{geo_lev}.csv"),
        index=False,
    )
