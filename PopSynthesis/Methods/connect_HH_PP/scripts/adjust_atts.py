...
def wrapper_adjust_state(syn_pop, dict_diff, processed_atts, main_att, pool, geo_lev):
    ...
    for zone in dict_diff:
        ...
        for neg_state in ls_neg_states:
            print(f"DOING adjustment for {main_att}_{neg_state} for zone {zone} with val: {count_vals[neg_state]}")
            comb_count_neg = get_comb_count(main_att, neg_state, processed_atts, sub_syn_pop, normalize=False)
            ...
            if count_vals[neg_state] < 0:
                ...
                print(f"WARNING: could not do total adjustment for {main_att}_{neg_state}, still have {count_vals[neg_state]}")
