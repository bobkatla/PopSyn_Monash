...
def main():
    ...
    selected_hh_for_weights = hh_df_raw.select(
        pl.col("hhid", "wdhhwgt_sa3", "wehhwgt_sa3")
    )
    selected_pp_for_weights = hh_df_raw.select(
        pl.col("persid", "wdperswgt_sa3", "weperswgt_sa3")
    )
    ...
    if files_exist:
        ...
        hh_df.to_csv(
            os.path.join(processed_data, "before_process_more_hh.csv"), index=False
        )
        pp_df.to_csv(
            os.path.join(processed_data, "before_process_more_pp.csv"), index=False
        )
    ...
    dict_hh_state_names = {
        ...
        if hh_cols not in ALL_RELA and hh_cols not in NOT_INCLUDED_IN_BN_LEARN
    }
    ...
    df_hh_main = process_hh_main_person(
        hh_df, main_pp_df, to_csv=True, include_weights=False
    )
    ...
    for rela in ALL_RELA:
        if rela != "Self":
            ...
            df_main_other = process_main_other(
                main_pp_df, sub_df, rela=rela, to_csv=True, include_weights=False
            )
