...
def convert_full_to_marg_count(
    full_pop: pd.DataFrame, geo_lev: str, filter_ls: list[str]
) -> pd.DataFrame:
    ...
    ls_drop_m = list(
        new_marg_hh.columns[new_marg_hh.columns.get_level_values(0).isin(["zone_id"])]
    )
...
def adjust_kept_hh_match_census(kept_hh, diff_census, geo_lev):
    ...
    for zone, r in diff_census.iterrows():
        ...
        before_sum = count_kept.loc[
            count_kept.index.get_level_values(geo_lev) == zone
        ].sum()
        ...
        logger.info(
            f"FNISHED deleting {before_sum - after_sum} HHs to match cencus diff for {zone}"
        )
