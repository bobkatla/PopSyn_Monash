...
main_pp_df_hh = pd.read_csv(
    os.path.join(processed_data, f"SynPop_hh_main_{geo_lev}.csv")
)
...
for file in glob(os.path.join(processed_data, "pp*")):
    ...
    cols_main = [
        f"{x}_main" for x in PP_ATTS if x not in ["relationship", "persid", "hhid"]
    ]
    rename_cols = {
        f"{name}_{rela}": name
        for name in PP_ATTS
        if name not in ["relationship", "persid", "hhid"]
    }
