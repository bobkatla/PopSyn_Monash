...
def _learn_para_BN(model, data_df):
    para_learn = BayesianEstimator(model=model, data=data_df)
            ...
            if len(evidence) > 1:
                ...
                for i in range(len(evidence)):
                    syn = inference.rejection_sample(
                        evidence=evidence[i], size=ls_sz[i], show_progress=True
                    )
                    ls_re.append(syn)
            else:
                syn = inference.rejection_sample(
                    evidence=evidence[0], size=n_hhsz, show_progress=True
                )
                ls_re.append(syn)
        if ls_re == []:
            continue
