...
from PopSynthesis.Methods.BN.TBN.utils import (
...
def learn_para_BN_dirichlet(model, data_df, state_names, prior_counts):
    ...
    ls_CPDs = para_learn.get_parameters(
        prior_type="dirichlet", pseudo_counts=prior_counts
    )
...
def learn_BN_diriclet(data_df, con_df, tot_df):
    ...
    model = learn_struct_BN_score(
        data_df, state_names=state_names, scoring_method="bicscore", show_struct=False
    )
...
def sample_BN(model, n, typeOf="forward", evidence=None):
    ...
    if typeOf == "forward":
        if evidence:
            print(
                "Using forward sampling, the evidence you provided would not have any effect"
            )
...
def cross_entropy(dist_target, dist_check):
    ...
    for i in range(len(dist_target)):
        result += dist_target[i] * math.log(
            dist_check[i], 2
        )  # in bits, change to e if want nats
    return -result
...
def mutation(
    indi, BN_model, con_df, tot_df, partition_rate=0.25, num_keep_atts=3, num_child=5
):
    arr_solutions = []
    ...
    for _ in range(num_child):
        ...
        for state_combine in count_combine_best_att.index:
            ...
            # create evidence
            evidence = [
                State(att, state) for att, state in zip(ls_best_atts, state_combine)
            ]
...
def EvoProg_check_loop(
    ...
    model = learn_BN_diriclet(
        seed_data, con_df, tot_df
    )  # NOTE: this model is quite good as it does incorporate census data
    ...
    while counter < num_gen and err_score >= err_converg:
        ...
        for i in range(len(solutions)):
            mutation_offsp = mutation(
                ...
                num_keep_atts=int(
                    len(state_names) / 3
                ),  # A more robust way/ dynamic to declare this
                num_child=(
                    num_pop - i
                ),  # this is to make sure that the population size is correct
            )
        ...
        for _ in range(crossover_time):
            ...
            cross_offsp = crossover(
                pa1=best_pa_sol[0], pa2=best_pa_sol[1], partition_rate=0.4
            )
            solutions.extend(cross_offsp)
        # Select the "best" for next round (or replacement)
        sorted_solutions = eval_ls_solutions(
            solutions, con_df, tot_df, n=len(solutions)
        )
        ...
        model = learn_para_BN_dirichlet(
            model, sorted_solutions[0], state_names, prior_counts
        )
        counter += 1
...
def main():
    ...
    start = time.time()
    final_pop = EvoProg_check_loop(
        seed_data, ori_data, con_df, tot_df, num_gen=20, random_rate=0.3
    )
    end = time.time()
