...
def cal_count_states(con_df, tot_df):
    ...
    for att in state_names:
        ...
        for state in ls_states:
            tot_name = con_df[(con_df["att"] == att) & (con_df["state"] == state)][
                "tot_name"
            ]
...
def learn_struct_BN_score(
    ...
    if show_struct:
        nx.draw_circular(model, with_labels=True, node_size=2000, font_size=12)
        plt.show()
    return model


def learn_para_BN(model, data_df, learn_method=BayesianEstimator, state_names=None):
    para_learn = learn_method(model=model, data=data_df, state_names=state_names)
...
def get_prior(raw_model, con_df, tot_df):
    ...
    for att in final_counts:
        pa = sorted(
            raw_model.get_parents(att)
        )  # DO this to match with the way they do it in pgmpy, dunno why they sort
        att_probs = final_counts[att]["probs"]
        vals_2d_matrix = []
        for prob in att_probs:
            ls_vals_states = [[prob]]
            for p in pa:
                ls_vals_states.append(final_counts[p]["probs"])
