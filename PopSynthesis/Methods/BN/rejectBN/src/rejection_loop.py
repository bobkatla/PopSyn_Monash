...
def reject_loop(BN_model, df_sample, census, sa_level, control_df):
    ...
    for zone in census[sa_level]:  # change this later
        ...
        att_done = (
            []
        )  # this is to store atts we have optimised so we can use them as evidence again
        while extra_n > 0:
            extra_df = reject_sample(BN_model, n=extra_n)
            if extra_df:
                ls_df.append(extra_df)
            final_df = pd.concat(ls_df)
            err_list = err_check(
                final_df, census[zone], control_df
            )  # THIS IS WRONG BUT JUST LEFT IDEA HERE
