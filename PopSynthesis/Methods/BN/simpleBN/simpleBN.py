...
def loop_learn_full_pop(
    loc_data, range_sample=np.linspace(0.01, 0.1, 10), reject_sample=True
):
    ...
    for rate in range_sample:
            ...
            for evidence in marginals.index:
                if evidence[0] == name_check_evi:
                    ...
                    if evidence in marg_seed:
                        re_sample_df = inference.rejection_sample(
                            evidence=[State(name_check_evi, evidence[1])],
                            size=sample_n,
                            show_progress=True,
                        )
                    else:
        ...
        results_rmse.append(
            compare_RMS_census(marginals, get_marg_val_from_full(syn_pop))
        )
...
def main():
    ...
    results, results_rmse = loop_learn_full_pop(
        loc_data=loc_data,
    ...
    print(results)
    np.save(
        f"./output/results_rmse_simpleBN_{min_rate}_{max_rate}.npy",
        np.asarray(results_rmse),
    )
    print(results_rmse)
