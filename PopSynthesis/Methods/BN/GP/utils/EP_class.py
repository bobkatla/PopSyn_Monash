...
class EP_BN_creator(EP_base):
    def __init__(
        self, sample: pd.DataFrame, marginal: pd.DataFrame, controls: pd.DataFrame
    ) -> None:
        super().__init__()
        self.sample = sample
...
class EP_for_full_pop_creator(EP_base):
    def __init__(self, loc_data) -> None:
        ...
        self.full_df_hh = realise_full_pop_based_on_weight(
            self.seed_df_hh, weight_col="wdhhwgt_sa3"
        )
    ...
    def loop_check(self, range_sample=np.linspace(0.01, 0.1, 10)):
        ...
        for rate in range_sample:
            ...
            SRMSE = SRMSE_based_on_counts(
                self.full_df_hh.value_counts(), syn_pop.value_counts()
            )
            results.append(SRMSE)
            results_rmse.append(
                compare_RMS_census(self.marginals, get_marg_val_from_full(syn_pop))
            )
            print(f"Done rate {rate} with score of {SRMSE}")
        return results, results_rmse

    def run_EP(
        self, seed_data, num_pop=8, random_rate=0.2, num_gen=20, crossover_time=2
    ):
        ...
        while counter < num_gen:
            ...
            for _ in range(crossover_time):
                ...
                self.cross_over(
                    pa1=best_pa_sol[0][0], pa2=best_pa_sol[1][0], partition_rate=0.4
                )
...
def test():
    ...
    results, results_rmse = EP_creator.loop_check(
        range_sample=np.linspace(min_rate, max_rate, tot)
    )

    np.save(f"{output_loc}/result_EP_{min_rate}_{max_rate}.npy", np.asarray(results))
    print(results)
    np.save(
        f"{output_loc}/result_EP_{min_rate}_{max_rate}.npy", np.asarray(results_rmse)
    )
    print(results_rmse)
