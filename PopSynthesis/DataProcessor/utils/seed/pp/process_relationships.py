...
class Household:
    AVAILABLE_RELATIONSHIPS = ["Main", "Spouse", "Child", "Grandchild", "Sibling", "Others", "Parent", "Grandparent"]
    ...
    def check_implausible(self) -> bool:
        ...
        for person in self.persons:
            ...
            implausible_parent = person.relationship == "Parent" and age_gap < MIN_PARENT_CHILD_GAP
            implausible_grandparent =  person.relationship == "Grandparent" and age_gap < MIN_GRANDPARENT_GRANDCHILD_GAP
            if implausible_child or implausible_grandchild or implausible_parent or implausible_grandparent:
    ...
    if household.is_implausible or highest_inc_rela == "Main":
        ...
        for other in household.segment_by_rela["Others"]:
            ...
            if num_spouse > 0:
                ...
                in_permited_age = highest_inc_person.age > MIN_PERMITTED_AGE_MARRIED and other.age > MIN_PERMITTED_AGE_MARRIED and age_gap <= MAX_COUPLE_GAP
        ...
        all_to_others_pp = household.segment_by_rela["Spouse"] + household.segment_by_rela["Child"] + household.segment_by_rela["Grandchild"]
...
def process_rela(pp_df: pl.DataFrame) -> pl.DataFrame:
    ...
    to_process_pp_df = process_chosen_to_others(pp_df) # Note, it's Others not Other, minor to help tell diff

    to_process_pp_df["converted_income"] = to_process_pp_df["persinc"].apply(convert_simple_income)
    to_process_pp_df["converted_persons"] = to_process_pp_df.apply(lambda r: Person(r["persid"], r["age"], r["sex"], r["converted_income"], r["relationship"]), axis=1)
    
    groups_by_hhid = to_process_pp_df.groupby("hhid")["converted_persons"].apply(lambda x: list(x))
    converted_hh_results = pd.DataFrame(groups_by_hhid)
    converted_hh_results["converted_households"] = converted_hh_results["converted_persons"].apply(lambda x: Household(x))

    # Still need to think about how to handle case of Other
    converted_hh_results["converted_households"] = converted_hh_results["converted_households"].apply(get_new_update_household)

    # Now we need to remove hh that has implauble case
    converted_hh_results["implausible_case"] = converted_hh_results["converted_households"].apply(lambda x: x.is_implausible)
    ...
    filtered_cases.loc[:, ["highest_inc_rela"]] = converted_hh_results["converted_households"].apply(lambda x: x.highest_income_person.relationship)
    assert (filtered_cases["highest_inc_rela"] == "Main").all() # assert all highest inc is Main

    # use the mapping to new rela based on cases
    filtered_cases.loc[:, ["new_mapping_from_id"]] = converted_hh_results["converted_households"].apply(lambda x: x.get_mapping_results_current())
    result_mapping = {}
