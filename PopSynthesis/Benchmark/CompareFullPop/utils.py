...
def condense_pop(pop_df: pd.DataFrame, name_for_weight_col: str) -> pd.DataFrame:
    ...
    if "hh_num" in final_df:
        final_df = final_df.drop(columns="hh_num")
    return final_df


def get_pp_based_on_id(
    hh_pop: pd.DataFrame, full_pp_pop: pd.DataFrame, shared_id_col: str
) -> pd.DataFrame:
...
def wrapper_get_all(
    seed_df_hh: pd.DataFrame,
    seed_df_pp: pd.DataFrame,
    sample_rate: float,
    name_weights_in_hh: str,
    new_name_weights_in_hh: str,
    shared_ids_name: str,
    ...
    fake_HH_seed_data = condense_pop(
        sample_of_pop, name_for_weight_col=new_name_weights_in_hh
    )
    # find a way to do it for both HH and PP
    fake_PP_seed_data = get_pp_based_on_id(
        fake_HH_seed_data, seed_df_pp, shared_ids_name
    )
...
def main():
    ...
    fake_HH_seed_data, fake_PP_seed_data = wrapper_get_all(
        seed_df_hh,
        seed_df_pp,
        sample_rate=0.01,
        name_weights_in_hh="wdhhwgt_sa3",
        new_name_weights_in_hh="_weight",
        shared_ids_name="hhid",
    )
