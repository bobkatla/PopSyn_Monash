...
def convert_2016_2021(df, sa_lev):
    ...
    convert_map = convert_map[[f"{old_name}_2016", f"{sa_lev}_CODE_2021"]]
    # The last row is problematic
    convert_map = convert_map[:-1]

    convert_map[f"{sa_lev}_CODE_2021"] = convert_map[f"{sa_lev}_CODE_2021"].astype(
        "float"
    )
    # convert_map = convert_map[convert_map[f"{old_name}_2016"]!=convert_map[f"{sa_lev}_CODE_2021"]]

    dict_check = dict(
        zip(convert_map[f"{old_name}_2016"], convert_map[f"{sa_lev}_CODE_2021"])
    )
...
def census_process_extra(df):
    ...
    df["TOT_CASUAL_GUESS"] = df["P_Tot_Emp_Tot"] - (
        df["P_Emp_PartT_Tot"] + df["P_Emp_FullT_Tot"]
    )
...
def get_census_sa(atts, sa_level, ls_filter_zone=None):
    ...
    inside_atts.extend([f"{sa_level}_CODE_2021", f"{sa_level}_NAME_2021"])
    gdf_p = gpd.read_file(
    ...
    gdf_num_p = gpd.read_file(
        f"{loc_file_census}G35_VIC_GDA2020.gpkg", layer=f"G35_{sa_level}_2021_VIC"
    )  # number people inside
    final_gdf = final_gdf.merge(gdf_num_p, how="inner")

    gdf_employ = gpd.read_file(
        f"{loc_file_census}G46B_VIC_GDA2020.gpkg", layer=f"G46B_{sa_level}_2021_VIC"
    )  # num employment
    ...
    df = df.rename(
        columns={f"{sa_level}_CODE_2021": sa_level, f"{sa_level}_NAME_2021": "NAME"}
    )
