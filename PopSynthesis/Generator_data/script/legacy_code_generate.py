...
def flatten_data_self_generate(
    ls_atts, make_like_paper=False, to_csv=False, csv_name="flatten_seed_data.csv"
):
    # import data
    p_original_df = pd.read_csv(
        "./data/source/VISTA_2012_16_v1_SA1_CSV/P_VISTA12_16_SA1_V1.csv"
    )
    ...
    h_original_df = pd.read_csv(
        "./data/source/VISTA_2012_16_v1_SA1_CSV/H_VISTA12_16_SA1_V1.csv"
    )
...
def generate_control_files_from_population(
    pop_df, to_csv=False, csv_name_control="flat_con.csv", csv_name_marg="flat_marg.csv"
):
    # NOTE: this is mainly use in case we have a whole population data, or testing using the seed data
    d_marg = {"total": len(pop_df)}
    d_con = {
        "target": [],
        "geography": [],
        "seed_table": [],
