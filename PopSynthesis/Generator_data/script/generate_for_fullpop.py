...
ATTRIBUTES = ['age', 'hhsize', 'relationship', 'carlicence', 'sex', 'persinc', 'dwelltype', 'totalvehs', 'wdhhwgt_sa3']


def flatten_connect_pp_hh(atts:list, hh_df:pd.DataFrame, pp_df:pd.DataFrame) -> pd.DataFrame:
    ...
    df_filter_only_self = df_atts[df_atts["relationship"]=="Self"] # NOTE: as these are people who do the survey so they will kinda be biased
    min_age, max_age = 0, 120 # NOTE: checked and min age is 15, max age is 116
    # Bin age into groups
    interval = 10 
    bins = pd.IntervalIndex.from_tuples([(begin, begin + interval) for begin in range(min_age, max_age+1, interval)])
    df_filter_only_self["age"] = pd.cut(df_filter_only_self["age"], bins).dropna() # This will give err if there are NaN
